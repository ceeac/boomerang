// Generated by Flexc++ V2.06.04 on Wed, 11 Apr 2018 13:29:23 +0200

#ifndef SSLScannerBASE_H_INCLUDED
#define SSLScannerBASE_H_INCLUDED
...
private:
    ...
    size_t (SSLScannerBase::*d_get)() = &SSLScannerBase::getInput;
...
protected:
    ...
    int     const (*d_dfaBase__)[94];

    static int     const s_dfa__[][94];
    static int     const (*s_dfaBase__[])[94];
...
public:
    SSLScannerBase(SSLScannerBase const &other)             = delete;
    SSLScannerBase &operator=(SSLScannerBase const &rhs)    = delete;
...
protected:
    SSLScannerBase(std::istream &in, std::ostream &out);
    SSLScannerBase(std::string const &infilename, std::string const &outfilename);
    ~SSLScannerBase();
...
inline SSLScannerBase::~SSLScannerBase()
{
    d_input.close();
}

// $insert startcondimpl
inline SSLScannerBase::StartCondition__ constexpr SSLScannerBase::SC(int sc)
{
    return as<StartCondition__>(sc);
}

inline int constexpr SSLScannerBase::SC(StartCondition__ sc)
{
    return as<int>(sc);
}

inline SSLScannerBase::StartCondition__ SSLScannerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void SSLScannerBase::begin(StartCondition__ startCondition)
{
...
template <typename ReturnType, typename ArgType>
inline ReturnType constexpr SSLScannerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

inline bool SSLScannerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr SSLScannerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &SSLScannerBase::out()
{
    return *d_out;
}

inline void SSLScannerBase::push(size_t ch)
{
    d_input.reRead(ch);
}

inline void SSLScannerBase::push(std::string const &str)
{
    d_input.reRead(str, 0);
}

inline void SSLScannerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void SSLScannerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &SSLScannerBase::matched() const
{
    return d_matched;
}

inline std::string const &SSLScannerBase::filename() const
{
    return d_filename;
}

inline void SSLScannerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t SSLScannerBase::length() const
{
    return d_matched.size();
}

inline void SSLScannerBase::leave(int retValue) const
{
    throw as<Leave__>(retValue);
}

inline size_t SSLScannerBase::lineNr() const
{
    return d_input.lineNr();
}

inline void SSLScannerBase::more()
{
    d_more = true;
}

inline size_t SSLScannerBase::state__() const
{
    return d_state;
}

inline size_t SSLScannerBase::get__()
{
    return (this->*d_get)();
}

inline size_t SSLScannerBase::getInput()
{
    return d_input.get();
}

inline bool SSLScannerBase::return__()
{
    return d_return;
}

inline void SSLScannerBase::noReturn__()
{
    d_return = false;
}


#endif //  SSLScannerBASE_H_INCLUDED
